...
void finalstat() {
    ...
    if(sys.sim_type==1) {
        printf("Printing TIS statistics to file\n");
        print_tisstats();
        reweight_crosshist();
...
void print_pathnumbers() {
    ...
    if(sys.sim_type==1) {
        ...
        int irep,jrep;
        int totpaths[path.nstates];
        int npaths[path.nstates][path.nstates];
        printf("\nPath numbers             ");
        for (i=0;i<path.nstates;i++) { 
            printf("%2d           ",i+1);
        }
        ...
        for (i=0;i<path.nstates;i++) { 
            ...
            for (k=0;k<path.nstates;k++) { 
                ...
                for(irep=0; irep<nrep; irep++) {
                    for(jrep=0; jrep<nrep; jrep++) {
                        npaths[i][k]+=state[i].pathtypenumbers[irep][k][jrep];
                    }
                }
                totpaths[i]+=npaths[i][k];
                printf("%12d ", npaths[i][k]);
            }  
            printf("%12d ", totpaths[i]);
            printf("\n");
        }
        printf("\nPath numbers Normalized  ");
        for (i=0;i<path.nstates;i++) { 
            printf("%2d           ",i+1);
        }
        printf("\n");
        for (i=0;i<path.nstates;i++) { 
            printf("     state  %2d ",i+1); 
            for (k=0;k<path.nstates;k++) { 
                printf("%12.5lf ", (double)npaths[i][k]/(double)totpaths[i]);
            }
            printf("\n");
        }
