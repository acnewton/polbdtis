...
void crossinghistogram(Replica *prep) {
	...
	for (i=0; i<path.nslices; i++) {
		z = slice[i].order_parameter[state_index];
		if (z > lambdamax) {
			lambdamax =z;
			if((maxrep+1)<nrep) {
...
void print_tisstats() {
    ...
    printf("Writing crosshistograms\n");
    ...
    printf("Writing path-type number histograms\n");
    for(istate=0; istate<path.nstates; istate++) {
    ...
    printf("Writing density of paths\n");
    if ((fp = fopen("dos_lambda.dat","w"))==NULL){
        ...
        for(istate=0; istate<path.nstates; istate++) {
            for(irep=0; irep<state[istate].nrep; irep++) {
                fprintf(fp,"%.16lf %.16lf\n",state[istate].srep[irep].lambda,state[istate].srep[irep].dos-state[istate].srep[1].dos);
            }
            fprintf(fp,"\n");
        }
        fclose(fp);
...
void reweight_pathtypenumbers() {
    ...
    //REMEMBER UNBOUND STATE IS STATE 0
	for(istate=0; istate<path.nstates; istate++) {
		printf("State %d\n",istate);
		nrep=1;
		...
		for(irep=1; irep<state[istate].nrep; irep++) {
			...
			for(jstate=0; jstate<path.nstates; jstate++) {
				for(krep=1; krep<=state[istate].nrep; krep++) {
					if(H[irep][jstate][krep]<0.01*maxvalue) {
						H[irep][jstate][krep]=0;
					}
					//nsum = Mi
	...
	for(ii=0; ii<path.nstates; ii++) {
        ...
        if(pathnumsym[ii][1][ii]>0) {
            lnZ[ii]=log(pathnumsym[1][1][ii]/pathnumsym[ii][1][ii]);
        }
        else {
            lnZ[ii]=0;
        }
	...
	for(istate=0; istate<path.nstates; istate++) {
		...
		f1 = (double)state[istate].flux1/state[istate].nflux1;		
		flux = 1./(f0+f1);
